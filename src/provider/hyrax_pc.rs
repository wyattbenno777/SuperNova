...
use crate::{
  ...
  provider::pedersen::{
    ...
    CommitmentKeyExtTrait, CommitmentKey as PedersenCommitmentKey,
  ...
  traits::{
    ...
    Group, TranscriptEngineTrait, TranscriptReprTrait, AbsorbInROTrait, ROTrait
  }
};
...
impl<G: Group> CommitmentTrait<G> for HyraxCommitment<G> {
  ...
  fn to_coordinates(&self) -> (G::Base, G::Base, bool) {
    ...
    let mut x = G::Base::ZERO;
    let mut y = G::Base::ZERO;
    let mut is_infinity = false;
  
    for c in &self.comm {
      let (xc, yc, ic) = c.to_coordinates();
      x = x + xc;
      y = y + yc;
      is_infinity |= ic; 
    }
...
impl<G: Group> AbsorbInROTrait<G> for HyraxCommitment<G> {
  ...
  CommitmentKey<G>: CommitmentKeyExtTrait<G>,
{
  ...
  fn prove(
    ...
    // Commit to LZ
    let ck = ck as &PedersenCommitmentKey<G>;
    let com_LZ = PedersenCommitmentEngine::commit(ck, &LZ);
  ...
  fn verify(
    ...
    let ck = PedersenCommitmentEngine::reinterpret_commitments_as_ck(&comm);
    ...
    arg
      .ipa
      .verify(&vk.ck_v, &vk.ck_s, R.len(), &ipa_instance, transcript)*/
    Ok(())
  }
}